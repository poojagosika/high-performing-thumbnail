...
function Dashboard() {
  ...
  const deleteDialog = useDialog(!!deleteId, () => setDeleteId(null), { labelledBy: "dash-delete-title" });
  const bulkDeleteDialog = useDialog(bulkDeleteOpen, () => setBulkDeleteOpen(false), { labelledBy: "dash-bulk-delete-title" });
  const bulkTagDialog = useDialog(bulkTagOpen, () => setBulkTagOpen(false), { labelledBy: "dash-bulk-tag-title" });
  const tagPanelDialog = useDialog(tagPanelOpen, () => setTagPanelOpen(false), { labelledBy: "dash-tag-panel-title" });
  const tagDeleteDialog = useDialog(!!tagDeleteTarget, () => setTagDeleteTarget(null), { labelledBy: "dash-tag-delete-title" });
  const purgeDialog = useDialog(!!(purgeId || bulkPurgeOpen), () => { setPurgeId(null); setBulkPurgeOpen(false); }, { labelledBy: "dash-purge-title" });
  const bulkCollectionDialog = useDialog(bulkCollectionOpen, () => setBulkCollectionOpen(false), { labelledBy: "dash-bulk-collection-title" });
  const deleteCollectionDialog = useDialog(!!deleteCollectionId, () => setDeleteCollectionId(null), { labelledBy: "dash-delete-collection-title" });
  const zoomDialog = useDialog(!!zoomUrl, () => setZoomUrl(null), { label: "Zoomed thumbnail" });
  ...
  const handleExportReport = () => {
    ...
    ctx.fillText("Thumbnail Analytics Report", 48, 60);
    ...
    ctx.fillStyle = "#61616b";
    ctx.font = "11px Inter, sans-serif";
    ctx.fillText(`Generated by Thumbnail · ${allTags.length} unique tags · ${thumbnails.length} thumbnails`, 48, h - 24);

    canvas.toBlob((blob) => {
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "thumbnail-report.png";
      a.click();
      URL.revokeObjectURL(url);
  ...
  const handleKeyDown = useCallback(
    ...
    [compareMode, selectMode, view, setParams],
  );
      ...
      <main id="main" className="max-w-6xl mx-auto px-6 py-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-4 mt-1">
                ...
                <span className="flex items-center gap-1.5 text-[12px] text-[#7b7b88] capitalize">
                ...
                <>
                  <div className="rounded-xl border border-white/6 bg-[#111118] p-4">
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <Image className="w-3.5 h-3.5 text-[#7b7b88]" />
                      <span className="text-[12px] text-[#7b7b88]">Total</span>
                    </div>
                  ...
                  <div className="rounded-xl border border-white/6 bg-[#111118] p-4">
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <TrendingUp className="w-3.5 h-3.5 text-[#7b7b88]" />
                    ...
                    <span className="font-heading text-xl font-semibold text-white">
                      ...
                      {avgScore != null && (
                        <span className="text-[11px] text-[#61616b] font-normal ml-0.5">
                          /100
                        </span>
                      )}
                    </span>
                  ...
                  <div className="rounded-xl border border-white/6 bg-[#111118] p-4">
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <Trophy className="w-3.5 h-3.5 text-[#7b7b88]" />
                      <span className="text-[12px] text-[#7b7b88]">
                        Best Score
                      </span>
                    </div>
                    {best ? (
                      <div>
                        ...
                        <p className="text-[11px] text-[#61616b] truncate mt-0.5">
                          {best.title}
                        </p>
                      </div>
                    ) : (
                  ...
                  <div className="rounded-xl border border-white/6 bg-[#111118] p-4">
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <Star className="w-3.5 h-3.5 text-[#7b7b88]" />
                      <span className="text-[12px] text-[#7b7b88]">
                        Starred
                      </span>
                    </div>
        ...
        {!loading && thumbnails.length > 0 && (
          ...
          >
            <button
              ...
              className="flex items-center gap-1.5 text-[12px] text-[#7b7b88] hover:text-white border border-white/8 hover:border-white/12 rounded-lg px-3 py-1.5 transition-colors"
            >
          ...
          <div className="flex items-center gap-2">
            {!loading && thumbnails.length >= 2 && !selectMode && (
              <Button
                ...
                variant="outline"
                className={`h-9 text-[13px] font-medium gap-1.5 ${compareMode ? "border-emerald-500/30 text-emerald-400 hover:text-emerald-300 hover:border-emerald-500/40 bg-emerald-500/5" : "border-white/8 text-[#7b7b88] hover:text-white hover:border-white/12 bg-transparent"}`}
              >
                {compareMode ? (
                  <>
            ...
            {!loading && thumbnails.length > 0 && !compareMode && (
              <Button
                ...
                variant="outline"
                className={`h-9 text-[13px] font-medium gap-1.5 ${selectMode ? "border-blue-500/30 text-blue-400 hover:text-blue-300 hover:border-blue-500/40 bg-blue-500/5" : "border-white/8 text-[#7b7b88] hover:text-white hover:border-white/12 bg-transparent"}`}
              >
                {selectMode ? (
                  <>
                    <X className="w-3.5 h-3.5" />
            ...
            <Button
              ...
              variant="outline"
              className={`h-9 text-[13px] font-medium gap-1.5 ${activityOpen ? "border-white/16 text-white bg-white/5" : "border-white/8 text-[#7b7b88] hover:text-white hover:border-white/12 bg-transparent"}`}
            >
              ...
              <div className="rounded-xl border border-white/6 bg-[#111118] p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="flex items-center gap-1.5 text-[14px] font-medium text-white">
                    <Activity className="w-4 h-4 text-[#7b7b88]" />
                    Recent Activity
                  </h3>
                  <button
                ...
                {activities.length === 0 ? (
                  <p className="text-[13px] text-[#61616b]">
                  ...
                  <div className="space-y-0.5 max-h-64 overflow-y-auto scrollbar-none">
                    {activities.map((a) => {
                      const icons = {
                        ...
                        unstarred: <Star className="w-3 h-3 text-[#61616b]" />,
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[10px] text-[#61616b] shrink-0">
                            {timeAgo(a.createdAt)}
                          </span>
                        </div>
                      );
            ...
            <aside className="w-full lg:w-52 shrink-0 lg:sticky lg:top-6">
              <div className="rounded-xl border border-white/6 bg-[#111118] p-3">
                <div className="flex items-center justify-between px-1 mb-2">
                  <span className="text-[11px] uppercase tracking-wider font-medium text-[#61616b]">
                    Collections
                  </span>
                  <button
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-[13px] transition-colors ${
                    !activeCollection && !trashMode
                      ...
                      : "text-[#7b7b88] hover:bg-white/4 hover:text-white"
                  }`}
                >
                  ...
                  >
                    {renameId === c._id ? (
                      <input
                        ...
                        className="w-full h-8 px-2 rounded-lg border border-white/12 bg-white/3 text-[13px] text-white outline-none focus:border-white/16 transition-colors"
                      />
                    ) : (
                      <button
                      ...
                      >
                        ...
                        <span className="text-[11px] text-[#61616b] group-hover:hidden">
                    ...
                    {renameId !== c._id && (
                      <div className="absolute right-1.5 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="text-[#61616b] hover:text-red-400 transition-colors"
                ...
                {addingCollection && (
                  <input
                    ...
                    className="w-full h-8 mt-1 px-2 rounded-lg border border-white/12 bg-white/3 text-[13px] text-white placeholder:text-[#7b7b88] outline-none focus:border-white/16 transition-colors"
                  />
                )}
                ...
                {collections.length === 0 && !addingCollection && (
                  <p className="px-2 py-2 text-[12px] text-[#61616b] leading-relaxed">
        ...
        {!loading && withPerformance.length > 0 && (
          ...
          >
            ...
            {scoreAccuracy && (
              <p className="text-[12px] text-[#61616b] mb-4">
              ...
              {withPerformance.slice(0, 5).map((t) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[13px] text-[#7b7b88] group-hover:text-white transition-colors truncate w-28 shrink-0">
                      {t.title}
                    </span>
                    ...
                    <span className="text-[11px] text-[#61616b] w-12 text-right shrink-0 tabular-nums">
        ...
        {!loading && thumbnails.length > 0 && (
          ...
          >
            <div className="relative flex-1" ref={searchDropdownRef}>
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#61616b] pointer-events-none z-10" />
              <input
                ref={searchRef}
                type="text"
                value={search}
                ...
                className="w-full h-9 pl-9 pr-3 rounded-lg border border-white/8 bg-white/3 text-[14px] text-white placeholder:text-[#7b7b88] outline-none focus:border-white/16 transition-colors"
              />
                      ...
                      {s.type === "thumbnail" ? (
                        ...
                        <>
                          <div className="w-10 h-6 rounded bg-white/4 flex items-center justify-center shrink-0">
                            <Tag className="w-3 h-3 text-[#7b7b88]" />
                          </div>
                          <span className="text-[13px] text-[#7b7b88] truncate flex-1">
                            {s.label}
                          </span>
                          <span className="text-[10px] text-[#61616b] shrink-0">
                            Tag
            ...
            {!trashMode && (
            <button
              aria-label={showStarred ? "Show all thumbnails" : "Show only starred"}
              ...
              className={`shrink-0 h-9 w-9 rounded-lg border flex items-center justify-center transition-all ${
                showStarred
                  ...
                  : "border-white/8 bg-white/3 text-[#61616b] hover:text-amber-400 hover:border-white/12"
              }`}
            ...
            <div className="relative shrink-0">
              <ArrowUpDown className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#61616b] pointer-events-none" />
              <select
                value={sort}
                onChange={(e) => setParams({ sort: e.target.value, page: 1 })}
                className="h-9 pl-9 pr-3 rounded-lg border border-white/8 bg-white/3 text-[13px] text-[#7b7b88] outline-none focus:border-white/16 transition-colors appearance-none cursor-pointer"
              >
            ...
            <div className="relative shrink-0">
              <CalendarDays className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#61616b] pointer-events-none" />
              <select
                value={dateRange}
        ...
        {!loading && !trashMode && allTags.length > 0 && (
          ...
          >
            <span className="text-[12px] text-[#61616b] shrink-0">Tags</span>
            <button
              onClick={() => setTagPanelOpen(true)}
              className="shrink-0 flex items-center gap-1 rounded-full border border-white/8 px-2.5 py-1 text-[12px] text-[#7b7b88] hover:text-white hover:border-white/12 transition-colors"
            >
            ...
            <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className={`shrink-0 rounded-full px-2.5 py-1 text-[12px] font-medium transition-all ${
                    activeTags.includes(tag)
                      ...
                      : "bg-white/4 text-[#7b7b88] hover:text-white hover:bg-white/8"
                  }`}
                >
                  {tag}
                </button>
              ...
              {activeTags.length > 0 && (
                <button
                  ...
                  className="shrink-0 flex items-center gap-1 rounded-full px-2.5 py-1 text-[12px] text-[#7b7b88] hover:text-white transition-colors"
                >
                  <X className="w-3 h-3" />
                  Clear
                </button>
        ...
        {loading || (trashMode && trashed === null) ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <Trash2 className="w-5 h-5 text-[#61616b] mb-3" />
            <p className="text-[14px] text-[#7b7b88]">Trash is empty</p>
            <p className="text-[13px] text-[#61616b] mt-1">
          ...
          >
            <div className="w-12 h-12 rounded-xl bg-white/4 border border-white/6 flex items-center justify-center mb-4">
              <Image className="w-5 h-5 text-[#7b7b88]" />
            </div>
            <h2 className="font-heading text-lg font-medium text-white mb-1">
              No thumbnails yet
            </h2>
            <p className="text-[14px] text-[#7b7b88] max-w-sm mb-6">
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            {showStarred ? (
              <Star className="w-5 h-5 text-[#61616b] mb-3" />
            ) : dateRange !== "all" ? (
              <CalendarDays className="w-5 h-5 text-[#61616b] mb-3" />
            ) : (
              <Search className="w-5 h-5 text-[#61616b] mb-3" />
            )}
            <p className="text-[14px] text-[#7b7b88]">
              {showStarred
            ...
            {(activeTags.length > 0 ||
              ...
              <button
                ...
                className="text-[13px] text-[#7b7b88] hover:text-white mt-2 transition-colors"
              >
                Clear filters
              </button>
            )}
          ...
          >
            {/* Table header */}
            <div className={`grid ${canDrag ? "grid-cols-[20px_44px_1fr_80px_1fr_100px_40px]" : "grid-cols-[44px_1fr_80px_1fr_100px_40px]"} items-center gap-3 px-4 py-2.5 border-b border-white/6 text-[11px] text-[#61616b] uppercase tracking-wider font-medium`}>
              {canDrag && <span />}
              <span />
              <span>Title</span>
              <span>Score</span>
            ...
            {paginated.map((thumb, i) => (
              ...
              >
                {/* Drag handle */}
                {canDrag && (
                  <div className="text-[#61616b] group-hover:text-[#7b7b88] transition-colors">
                ...
                <div className="flex gap-1 overflow-hidden">
                  {thumb.tags?.slice(0, 3).map((tag) => (
                    <span
                      key={tag}
                      className="text-[10px] rounded-full px-2 py-0.5 bg-white/4 text-[#61616b] truncate shrink-0"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <button
                  ...
                  className={`transition-colors ${
                    thumb.starred
                      ? "text-amber-400"
                      : "text-[#61616b] opacity-0 group-hover:opacity-100 hover:text-amber-400"
                  }`}
                >
                  <Star
                    className="w-3.5 h-3.5"
              ...
              >
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {trashMode ? (
                      <span
                        className="text-[11px] text-[#61616b] shrink-0"
                          ...
                          <button
                            ...
                            className={`text-[11px] rounded-full px-2 py-0.5 transition-colors ${
                              activeTags.includes(tag)
                                ...
                                : "bg-white/4 text-[#61616b] hover:text-[#7b7b88]"
                            }`}
                          >
                            {tag}
                          </button>
        ...
        {!loading && totalPages > 1 && (
          ...
          >
            <button
              ...
              disabled={safePage <= 1}
              className="h-8 w-8 rounded-lg border border-white/8 bg-white/3 flex items-center justify-center text-[#7b7b88] hover:text-white hover:border-white/12 transition-colors disabled:opacity-30 disabled:pointer-events-none"
            >
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                ...
                className={`h-8 w-8 rounded-lg text-[13px] font-medium transition-colors ${
                  p === safePage
                    ? "bg-white text-[#0a0a0f]"
                    : "border border-white/8 bg-white/3 text-[#7b7b88] hover:text-white hover:border-white/12"
                }`}
              >
                {p}
              </button>
        ...
        {bulkTagOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.96, y: 10 }}
              transition={{ duration: 0.2 }}
              {...bulkTagDialog.dialogProps}
              className="relative w-full max-w-sm rounded-xl border border-white/6 bg-[#111118] shadow-2xl p-5"
            >
              <h2
                id="dash-bulk-tag-title"
                className="font-heading text-[15px] font-semibold text-white"
              >
              ...
              <input
                ...
                className="w-full h-9 px-3 mt-3 rounded-lg border border-white/8 bg-white/3 text-[14px] text-white placeholder:text-[#7b7b88] outline-none focus:border-white/16 transition-colors"
              />
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              ...
              {caseDuplicates.length > 0 && (
                <div className="px-5 py-3 border-b border-white/6 bg-amber-400/5">
                  ...
                  {caseDuplicates.map((group) => (
                    ...
                    >
                      <span className="text-[13px] text-[#7b7b88] flex-1 truncate">
                        {group.join("  ·  ")}
                      </span>
                      <button
              ...
              <div className="max-h-80 overflow-y-auto py-1">
                  ...
                  >
                    {tagRenameFrom === tag ? (
                      <input
                        ...
                        disabled={tagBusy}
                        className="flex-1 h-7 px-2 rounded-lg border border-white/12 bg-white/3 text-[13px] text-white outline-none focus:border-white/16 transition-colors"
                      />
                    ) : (
                      <>
                        <span className="text-[13px] text-white truncate flex-1">
                        ...
                        <span className="text-[11px] text-[#61616b] shrink-0">
                          {count}
                        </span>
                      </>
                    )}
                ...
                {tagStats.length === 0 && (
                  <p className="px-5 py-6 text-center text-[13px] text-[#61616b]">
                    No tags yet
                  </p>
                )}
              </div>
        ...
        {tagDeleteTarget && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
            ...
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.96, y: 10 }}
              transition={{ duration: 0.15 }}
              {...tagDeleteDialog.dialogProps}
              className="relative w-full max-w-sm rounded-xl border border-red-500/20 bg-[#111118] p-5"
            >
              <h3
                id="dash-tag-delete-title"
                className="font-heading text-[15px] font-semibold text-white mb-1"
              >
                Remove &ldquo;{tagDeleteTarget}&rdquo;?
              </h3>
              <p className="text-[13px] text-[#7b7b88] mb-5">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              ...
              <div className="max-h-56 overflow-y-auto -mx-1 px-1">
                {collections.map((c) => (
                  <button
                    ...
                    disabled={bulkMoving}
                    className="w-full flex items-center gap-2 px-2 py-2 rounded-lg text-[13px] text-[#7b7b88] hover:bg-white/4 hover:text-white transition-colors disabled:opacity-40"
                  >
                    <Folder
                      className="w-3.5 h-3.5 shrink-0"
                      style={{ color: c.color }}
              ...
              {collections.length === 0 && (
                <p className="text-[13px] text-[#61616b] py-2">
        ...
        {deleteCollectionId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.96, y: 10 }}
              transition={{ duration: 0.15 }}
              {...deleteCollectionDialog.dialogProps}
              className="relative w-full max-w-sm rounded-xl border border-white/8 bg-[#111118] p-5"
            >
              <h3
                id="dash-delete-collection-title"
                className="font-heading text-[15px] font-semibold text-white mb-1"
              >
                Delete collection?
              </h3>
